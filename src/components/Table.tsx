...
export function Table() {
  ...
  const renderBadge = (hasDone: boolean, status: string) => {
    if (!hasDone) {
      return (
        <span className="inline-flex items-center justify-center rounded border border-yellow-800 bg-white px-1 py-2 text-sm font-semibold leading-none text-yellow-800">
          {status}
        </span>
      );
    }
    return (
      <span className="inline-flex items-center justify-center rounded border border-green-800 bg-white p-1 text-sm font-semibold leading-none text-green-800">
        {status}
      </span>
    );
  };
      ...
      <div className="m-4">
        <div className="my-2 flex flex-1 gap-1">
          {Object.keys(new Array(5).fill(null))
            .map(Number)
            .map((_, index) => (
              <Skeleton key={index} className="h-6 w-3/12" />
            ))}
        </div>
  ...
  return (
    <Container>
      <table className="border-stroke shadow-default border-strokedark bg-boxdark mt-2 w-full overflow-y-auto border bg-white py-1 ">
        <thead className="bg-emerald-700 shadow">
          <tr className="text-center font-medium text-white">
            <th className="w-[48px] bg-white shadow-none" />
            <th className="min-w-[150px] border-x-2 px-2 py-4">STATUS</th>
            <th className="min-w-[150px] border-x-2">CPF</th>
            <th className="min-w-[150px] border-x-2 px-2">NOME</th>
            <th className="border-x-2 px-2">CIB</th>
            <th className="min-w-[150px] border-x-2 px-2">IMÓVEL RURAL</th>
            <th className="min-w-[150px] border-x-2 px-2">OBSERVAÇÕES</th>
          </tr>
        </thead>
        <tbody>
